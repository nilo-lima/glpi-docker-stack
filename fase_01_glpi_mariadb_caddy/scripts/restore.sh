...
# Verificar se existe o backup de arquivos correspondente
RESTORE_FILES=false
...
if [[ "${RESTORE_FILES}" == "true" ]]; then
    ...
    # Usa container temporário Alpine para extrair no volume sem precisar do glpi-app rodando
    VOLUME_NAME="${COMPOSE_PROJECT_NAME:-glpi-prod}_glpi_data"
    docker run --rm \
...
until docker inspect "${COMPOSE_PROJECT_NAME:-glpi-prod}-app" \
